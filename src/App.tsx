...
import { currentLegalMoves, newGame, tryMove, type GameState } from "./game";
  ...
  return (
    <main>
      ...
      <p className="status" role="status">
        {won ? "🎉 Solved!" : `Visited ${visited.length} / ${totalCells}`}
      </p>
      ...
      >
        {puzzle.available.flatMap((row, r) =>
          row.map((avail, c) => {
            const k = `${r}-${c}`;
            if (!avail) {
              return <div key={k} className="cell hole" aria-hidden="true" />;
            }
            const cell = { r, c };
            const dark = (r + c) % 2 === 1;
            ...
            return (
              <button
                ...
                aria-label={`square ${r},${c}${isEnd ? " (goal)" : ""}`}
              >
      ...
      <div className="controls">
        <button type="button" className="btn" onClick={handleNewPuzzle}>
          New puzzle
        </button>
      </div>
